...
public class Elevator implements Runnable {
    ...
    private final long doorTime = 200;
    private final long floorTime = 400;
    ...
    private void outputArrive() {
        TimableOutput.println(String.format("ARRIVE-%d",curFloor));
    }
    ...
    public boolean onWay(PersonRequest pr) {
        if (state != State.SLEEP) {
            ...
            if (from < to) {
                if (state == State.UP &&
                        (direc == Direc.DOWN ||
                        direc == Direc.UP && from >= curFloor)) {
                    return true;
                }
                if (state == State.DOWN && direc == Direc.UP &&
                        from >= curFloor) {
                    return true;
                }
            } else {
                if (state == State.DOWN &&
                        (direc == Direc.UP ||
                        direc == Direc.DOWN && from <= curFloor)) {
                    return true;
                }                
                if (state == State.UP && direc == Direc.DOWN &&
                        from <= curFloor) {
                    return true;
                }
            }            
